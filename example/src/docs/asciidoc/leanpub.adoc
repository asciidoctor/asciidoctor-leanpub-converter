...
are some limitation which we can not work around.

* Colspan:  If an Asciidoctor table has a colspan this will result in an empty column being generated.
* Frame: Frame is ignored. Leanpub does not do vertical lines. THe use of horizontal lines is determined by
  horizontal alignment as well as the presence of headers and footer.
...
* Vertical alignment: Not currently supported. We might implement a workarond in future, if there is a community
